# KEE Specification v0.10.0

Status: Published Normative Baseline  
Version: 0.10.0  
Date: 2026-09-19  
SemVer classification: MINOR  
Supersedes as current version identifier: KEE v0.9.2  
Published public API: `normative-api/v0.10.0.json`

---

## 0. Document status and authority

This document is the published KEE v0.10.0 normative baseline and the current version identifier for the KEE public normative contract.

KEE v0.10.0 is a **Semantic Versioning MINOR release**. It carries one conformance-behaviour change: the Section 12.3 accountable-root constraint, normative since it was introduced and reported at `sh:Warning` by the reference shape, is escalated to `sh:Violation`. The escalation was scheduled by a decision record rather than promised relatively, and a release control enforces the schedule. The owning profile is Draft / Non-Normative and excluded from the public normative API, which is the ground on which this release is classified MINOR-compatible rather than MAJOR.

It adds no KEE-owned semantic term, no profile promotion, no Capability Level, and no requirement beyond the escalation. It restates Section 18.7 to record which canonical identifiers now resolve, which do not yet, and what a resolving one resolves to. Like v0.9.2, it is self-contained: a reader needs no other document to determine the ordinary normative meaning of a KEE obligation.
...
Such a vocabulary SHALL have a declared owner, stable identifiers, definitions for each value, open/closed status, normative status, an extension/versioning policy, and external mappings or explanatory references where useful. "Stable identifier" here means a governed, versioned identifier that does not change; where that identifier belongs to a persistent-identifier scheme entry, Section 18.7 governs what may be claimed about its registration, resolution, and persistence. Where a value corresponds to established representation prior art, the scheme SHOULD record an explicit mapping or explanatory reference rather than implying KEE invented the representation.
...
Registration status of these namespaces is governed by Section 18.7; they are registered and resolve at this baseline.
...
A declared namespace prefix is only a serialization convenience. It is not evidence that KEE publishes a governed vocabulary at that namespace. The registration and resolution status of the namespaces named in this section is governed by Section 18.7; at this baseline they are canonical for new writes and registered, and they resolve. The canonical JSON-LD context and active SHACL assets MUST avoid unused KEE-owned namespace declarations where practical; in particular, `governance#` and `contradiction#` MUST NOT be presented as current governed vocabulary families unless concrete canonical terms are actually defined there.
...
This obligation is normative at this baseline, and the reference shape reports it at `sh:Violation` severity. This release performs the escalation scheduled for v0.10.0. Earlier baselines reported the same obligation at `sh:Warning`, so that data which was structurally valid under them did not become structurally invalid without a release that says so; this release says so. A grantee that resolves to no accountable person or organization conformed structurally under v0.9.2 and does not conform under v0.10.0. Implementations claiming the Authority and Delegation profile and relying on warning-level behaviour must resolve their delegation chains. An implementation claiming the Authority and Delegation profile that relied on warning-level behaviour MUST resolve its delegation chains to conform at this baseline.
...
Each release MUST publish a machine-readable manifest of its public normative API. The manifest for this release is `normative-api/v0.10.0.json`.
...
### 18.7 Namespace registration and dereference
...
Registration under the W3ID project is merged. Resolution has been independently verified for the base identifier and for the namespaces it directly serves as RDF: `https://w3id.org/kee` and `https://w3id.org/kee/artifact` resolve, content-negotiated, to the published term declarations and the artifact-classification concept scheme respectively; `https://w3id.org/kee/confidence` resolves to the confidence-interpretation concept scheme.

Verification is not yet complete for every identifier this section scopes over. As of this baseline, the generic `https://w3id.org/kee/lifecycle` namespace and the profile identifiers of Section 5 (`https://w3id.org/kee/profile`) redirect to a human-readable page of the public repository rather than content-negotiating to RDF; this specification MUST NOT be read as claiming otherwise for those identifiers, and a future baseline SHOULD record when that changes. A consumer needing a machine-readable definition for a namespace not yet confirmed to resolve to RDF SHOULD read the corresponding SKOS scheme or shape graph from the repository directly rather than rely on dereferencing.

What a resolving identifier resolves to is governed rather than incidental:

- the unversioned identifier `https://w3id.org/kee` denotes the **current published baseline**, and its meaning changes only when a release is published;
- each published release also carries a **version identifier** denoting the vocabulary as that release published it, which does not change afterwards and is a frozen artifact under Section 18.9;
- the served term-declaration document names the version it is, so a consumer can tell which release it received without comparing bytes; and
- a resolving request content-negotiates: an RDF media type returns the published term declarations or concept scheme, and other requests return the published repository.

Two limits are normative, because resolution and persistence are different claims:

- **Resolution is not durable persistence.** That an identifier resolves today is verified for the identifiers named above. Durable operational persistence is a claim about future behaviour, and this specification, its profiles, and its release evidence MUST NOT describe any KEE identifier as guaranteed-persistent on the strength of a verified resolution alone.
- **A consumer MAY rely on dereferencing a confirmed identifier and SHOULD NOT rely on dereferencing exclusively, for any identifier.** A consumer whose correctness depends on obtaining a definition SHOULD also be able to read the term-declaration document `vocab/kee.ttl` and the SKOS and SHACL assets published with this specification, because a redirect service is an operational dependency that this specification does not control.

The identifiers remain canonical for new writes under Sections 8.2, 9.2, and 10, whether or not resolution for a given one has yet been confirmed; an identifier can be canonical without being resolvable, and confirming resolution for one identifier does not confirm it for another in the same family.

The Section 4.3 "stable identifiers" criterion is satisfied as a governed, versioned, non-changing identifier; it is additionally satisfied as a resolvable one for the specific identifiers named above. Control of the `w3id.org/kee` path is exercised through the registered redirect and is not exclusive control of the resolving infrastructure, which is why the persistence limit above holds regardless of which identifiers currently resolve.
...
v0.10.0 promotes no profile. Executable assets for a Draft profile remain design/structural evidence only and do not promote it.
...
Published v0.10.0 is immutable and MUST NOT be substantively rewritten in place. Future corrections or compatible/incompatible changes follow the governed pipeline:
...
3. **The Section 12.3 accountable-root escalation is performed at this baseline.** The reference shape reports the obligation at `sh:Violation`, and a release control asserts the severity against the baseline so the schedule cannot lapse unobserved. This item is closed.
...
5. **Durable persistence of the canonical identifiers is not established, and resolution is confirmed only for the base identifier and the artifact and confidence namespaces.** Registration is merged; the identifiers named in Section 18.7 resolve, the others in the same family do not yet content-negotiate to RDF, and Section 18.7 states which is which. Persistence over time is a claim about future behaviour that a verified resolution does not settle, and the version-identifier redirect rules are not yet registered.
...
- Escalates the Section 12.3 accountable-root constraint from `sh:Warning` to `sh:Violation` in the reference shape. This is the one conformance-behaviour change, and the reason this release is MINOR rather than PATCH.
- A grantee that is not a `prov:Person` or `prov:Organization` and resolves through no delegation chain to one conformed structurally under v0.9.2 and does not conform at this baseline. An implementation claiming the Authority and Delegation profile and relying on warning-level behaviour must resolve its delegation chains.
- Classified MINOR-compatible because the owning Authority and Delegation profile is Draft / Non-Normative and excluded from the public normative API. The public normative contract is unchanged.
- Restates Section 18.7: registration is merged; the base identifier, the artifact scheme, and the confidence scheme resolve and are verified, other identifiers in the family do not yet content-negotiate to RDF and this baseline says so; the unversioned identifier denotes the current published baseline; each release also carries a version identifier; and durable persistence remains an unestablished claim distinct from verified resolution.
- Adds no requirement beyond the escalation, no KEE-owned semantic term, no profile promotion, and no Capability Level. Requires no governed-data migration.
